		...
		for key, source := range e.Snapshots {
			...
			*to.Snapshots[key] = *source
			...
			if source.Children != nil {
				to.Children = make(map[string]bool)
				for key, value := range source.Children {
					to.Children[key] = value
				}
			}
...
func (in *BackupVolumeStatus) DeepCopyInto(out *BackupVolumeStatus) {
	*out = *in
	...
	if in.Backups != nil {
		...
		*out = make(map[string]*VolumeSnapshotBackup, len(*in))
		for key, val := range *in {
			var outVal *VolumeSnapshotBackup
			if val == nil {
				(*out)[key] = nil
			} else {
				in, out := &val, &outVal
	...
	if in.LastSyncedTimestmp != nil {
		in, out := &in.LastSyncedTimestmp, &out.LastSyncedTimestmp
		*out = (*in).DeepCopy()
	}
}
