...
func (m *VolumeManager) Create(name string, spec *types.VolumeSpec) (v *longhorn.Volume, err error) {
	...
	if spec.FromBackup != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("cannot get volume and backup name from backup URL %v: %v", spec.FromBackup, err)
		}
		...
		if err != nil {
			return nil, fmt.Errorf("cannot get backup volume %v from backup URL %v: %v", backupName, spec.FromBackup, err)
		}
		...
		backup, err := backupTarget.InspectVolumeSnapshotBackupMetadata(spec.FromBackup)
		if err != nil {
