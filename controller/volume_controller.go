...
func (vc *VolumeController) checkAndInitVolumeRestore(v *longhorn.Volume) error {
	...
	backup, err := backupTarget.InspectVolumeSnapshotBackupMetadata(v.Spec.FromBackup)
	if err != nil {
...
func (vc *VolumeController) getInfoFromBackupURL(v *longhorn.Volume) (string, string, error) {
	...
	backupName, volumeName, _, err := backupstore.DecodeMetadataURL(v.Spec.FromBackup)
	return backupName, volumeName, err
}
